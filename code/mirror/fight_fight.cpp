...
void fight_fight::on_btn_quit_clicked(void)
{
	...
	else
	{
        if (QMessageBox::Yes == QMessageBox::question(this,
                                                      QStringLiteral("提示"),
                                                      QStringLiteral("当前正在战斗中，逃跑将损失10%声望及金钱。")))
		{
...
void fight_fight::on_btn_start_clicked(void)
{
    ...
    ui.edit_display->setText(QStringLiteral("战斗开始"));
	...
	for (MapItem::iterator iter = m_bag_item->begin(); iter != m_bag_item->end(); iter++)
	{
		...
		if (itemItem != NULL)
		{
			if (itemItem->type == et_immediate_hp)
			{
                ...
                strTmp += QStringLiteral("\t 血:") + QString::number(itemItem->value);
                strTmp += QStringLiteral("\t 剩:") + QString::number(iter.value());
				ui.comboBox_hp->addItem(strTmp);
			}
			else if (itemItem->type == et_immediate_mp)
			{
				strTmp = itemItem->name;
                strTmp += QStringLiteral("\t 魔:") + QString::number(itemItem->value);
                strTmp += QStringLiteral("\t 剩:") + QString::number(iter.value());
...
void fight_fight::LoadDistribute()
{
	...
	if (!file.open(QIODevice::ReadOnly))
    {
        QMessageBox::critical(this,
                              ...
                              QStringLiteral("加载怪物分布列表失败，请重新运行游戏。"));
...
void fight_fight::Step_role_UsingItem_hp(void)
{
	...
	if (itemItem != NULL)
	{
		...
		strTmp = itemItem->name;
        strTmp += QStringLiteral("\t 血:") + QString::number(itemItem->value);
        strTmp += QStringLiteral("\t 剩:") + QString::number(m_bag_item->value(ID));
...
void fight_fight::Step_role_NormalAttack(void)
{
	...
	if (!ui.checkBox_concise->isChecked())
	{
        QString strTmp = QStringLiteral("你使用普通攻击，对") + monster_cur->name
            + QStringLiteral("造成伤害:") + QString::number(nTmp);
		ui.edit_display->append(strTmp);
	}
}
...
void fight_fight::Action_role(void)
{
	...
	if (monster_cur->hp_c <= 0)
	{
        ...
        ui.edit_display->append(QStringLiteral("获得经验:") + QString::number(monster_cur->exp));
	}
}
void fight_fight::Action_monster(void)
{
	...
	if (!ui.checkBox_concise->isChecked())
	{
        QString strTmp = monster_cur->name + QStringLiteral("砍了你一刀，造成")
            + QString::number(nTmp) + QStringLiteral("点伤害。");
		ui.edit_display->append(strTmp);
	}
